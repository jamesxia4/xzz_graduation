...
# io
parser.add_argument('--dataset_path', default='./i3d1',
                    help="path to dataset")
parser.add_argument('--annotation_path', default="Train.txt",
                    help="path to train annotation")
parser.add_argument('--annotation_path_test', default="test.txt",
...
class FeaturesWriter:
    ...
    def _init_video(self, video_name, dir):
        self.path = path.join(dir, "{video_name}.txt")
        self.dir = dir
        self.data = dict()
    ...
    def _is_new_video(self, video_name, dir):
        new_path = path.join(dir, "{video_name}.txt")
...
def read_features(video_name, dir):
    file_path = "{video_name}.txt"
    file_path = path.join(dir, file_path)
    if not path.exists(file_path):
        raise Exception("Feature doesn't exist: {file_path}")
    features = None
...
def main():
    ...
    #                                        batch_size=args.batch_size,
    #                                        shuffle=False,
    #                                        num_workers=4,  # 4, # change this part accordingly
    #                                        pin_memory=True)
