...
import socket
import logging
from utils import graph_generator_ft
import torch
...
class static_model(object):
        ...
        if strict:
            ...
            for name, param in state_dict.items():
                if "student_model" in name:
                    name = name.replace("student_model.", "")
                if "module" in name:
    ...
    def load_checkpoint(self, pretrain_path, epoch=0, optimizer=None):

        assert os.path.exists(pretrain_path), "Failed to load: {} (file not exist)".format(pretrain_path)

        checkpoint = torch.load(pretrain_path, map_location='cpu')

        all_params_matched = self.load_state(checkpoint['state_dict'], strict=False)  # True

        if optimizer:
            if 'optimizer' in checkpoint.keys() and all_params_matched:
                optimizer.load_state_dict(checkpoint['optimizer'])
                logging.info("Model & Optimizer states are resumed from: `{}'".format(pretrain_path))
            else:
                logging.warning(">> Failed to load optimizer state from: `{}'".format(pretrain_path))
        else:
            logging.info("Only model state resumed from: `{}'".format(pretrain_path))
    ...
    def forward(self, data, target):
        ...
        """
        adjf,adjt=graph_generator_ft(data) 
        data = data.float().cuda()
        adjf=adjf.float().cuda()
        adjt=adjt.float().cuda()
        target = target.cuda()
        if self.net.training:
            input_var = torch.autograd.Variable(data, requires_grad=False)           
            adjf_var = torch.autograd.Variable(adjf, requires_grad=False)
            adjt_var = torch.autograd.Variable(adjt, requires_grad=False)
            target_var = torch.autograd.Variable(target, requires_grad=False)
        else:
            with torch.no_grad():
                input_var = torch.autograd.Variable(data)
                adjf_var=torch.autograd.Variable(adjf)
                adjt_var=torch.autograd.Variable(adjt)
                target_var = torch.autograd.Variable(target)
        output,adj,vid= self.net(input_var,adjf_var,adjt_var)
        if hasattr(self, 'criterion') and self.criterion is not None \
                and target is not None:
            loss = self.criterion(output, target_var,adj,vid)
        else:
            loss = None
        return [output], [loss]
    ...
    def predict(self, data):   
        ...
        with torch.no_grad():
            input_var = torch.autograd.Variable(data)
            adj_varf=torch.autograd.Variable(adjf)
            adj_vart=torch.autograd.Variable(adjt)
        output,adj,vid = self.net(input_var,adjf,adjt)
...
class model(static_model):
    ...
    def fit(self, train_iter, optimizer, lr_scheduler,
        ...
        """
        pause_sec = 0.
        # writer=SummaryWriter(comment='GCNMIL')
        for i_epoch in range(epoch_start, epoch_end):
            ...
            # writer.add_scalar('Train',loss,i_epoch)
            ###########
            # 2] END OF EPOCH
            ###########
